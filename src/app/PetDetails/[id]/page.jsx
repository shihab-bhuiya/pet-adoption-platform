...
const PetDetailsPage = () => {
  ...
  // Mock user for the modal form (We will swap this with Better Auth session data next!)
  const currentUser = {
    name: "Shihab Bhuiya",
  ...
  // Fetch single pet data by ID from Express backend
  useEffect(() => {
    const fetchPetDetails = async () => {
      try {
        // Change this URL if your backend port or route matches something else
  ...
  const handleSubmitAdoption = async (e) => {
    ...
    try {
      // POST the adoption application details directly to backend
      const response = await axios.post('http://localhost:5000/api/adoption-requests', adoptionRequest);
